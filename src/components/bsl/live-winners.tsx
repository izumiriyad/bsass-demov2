...
export function LiveWinners() {
  ...
  return (
    <div className="relative flex h-10 w-full items-center overflow-hidden rounded-full bg-[#1b1c1e] px-4 shadow-[inset_0_1px_rgba(255,255,255,0.05)] border border-[#2a2c30]">
      <div className="flex shrink-0 items-center gap-2 pr-4 font-black uppercase text-[#ffdf19] border-r border-[#2a2c30] mr-4">
        ...
        <div className="flex animate-ticker items-center gap-8 whitespace-nowrap">
          {[...winners, ...winners, ...winners].map((w, i) => (
            <div key={`${w.id}-${i}`} className="flex items-center gap-2 text-sm shrink-0">
              ...
              <span className="text-[#2a2c30] select-none px-2">|</span>
            </div>
          ))}
        </div>
      </div>
